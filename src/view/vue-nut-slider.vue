<template>
    ...
    >
        ...
        <div class="nut-slider-pagination" v-show="paginationVisible">
...
export default {
    ...
    methods:{
        ...
        updateEvent(pageSize){
            this.$nextTick(()=>{
                this.domTimer = setTimeout(()=>{
                    ...
                    this.sliderWrap = this.$el.querySelector('.nut-slider-wrap');
                    this.slideEls = [...this.sliderWrap.children];
        ...
        _slideClassHandle(){
            let selectedSlide = this.$el.querySelector('.nut-slider-slide-selected');
            selectedSlide && selectedSlide.classList.remove('nut-slider-slide-selected');
            this.slideEls[this.currentPage -1].classList.add('nut-slider-slide-selected');
        ...
        _onTouchStart(e){
            this.sliderWrap.addEventListener('touchmove',this._onTouchMove,false);
            this.sliderWrap.addEventListener('touchend',this._onTouchEnd,false);
            this.sliderWrap.addEventListener('mousemove',this._onTouchMove,false);
            this.sliderWrap.addEventListener('mouseup',this._onTouchEnd,false);
        ...
        _onTouchEnd(e){
            ...
            this.sliderWrap.removeEventListener('touchmove',this._onTouchMove,false);
            this.sliderWrap.removeEventListener('touchend',this._onTouchEnd,false);
            this.sliderWrap.removeEventListener('mousemove',this._onTouchMove,false);
            this.sliderWrap.removeEventListener('mouseup',this._onTouchEnd,false);
        },
        _revert(){
        ...
        _createLoop(){
            let propName = this.isHorizontal() ? 'offsetWidth' : 'offsetHeight';
            let sliderWrapEl = this.$el.querySelector('.nut-slider-wrap');
        ...
        _imgLazyLoad(type){
            ...
            let imgLazyLoadEl;
            requestAniFrame(()=>{
                imgLazyLoadEl = this.sliderWrap.querySelectorAll('.nut-img-lazyload');
                if(type == 1){
        ...
        _checkInView(imgItem){
            ...
            let sliderRect = this.$el.getBoundingClientRect();
            let imgTop = imgRect.top;
            let imgLeft = imgRect.left;
            let sliderWidth = this.$el.clientWidth;
            let sliderHeight = this.$el.clientHeight;
            let sliderTop = sliderRect.top;
            let sliderLeft = sliderRect.left;
            let isInView  = true;
            if(imgTop > sliderTop + sliderHeight || imgLeft > sliderLeft + sliderWidth){
                isInView = false;
            }
            return isInView;
        }
...
.nut-slider{
    ...
    &.horizontal .nut-slider-wrap{
        flex-direction: row;
        -webkit-flex-direction:row;
    }
    &.vertical .nut-slider-wrap{
        flex-direction: column;
        -webkit-flex-direction:column;
    }
    .nut-slider-pagination{
        position: absolute;
    ...
    &.vertical .nut-slider-pagination{
        right:10px;
        top:50%;
    ...
    &.horizontal .nut-slider-pagination{
        bottom:10px;
        width:100%;
        text-align: center;
        .slider-pagination-bullet{
